...
public class ShortifySpoutListener extends GenericShortifyListener implements Listener {
	...
	public void playerChat(PlayerChatEvent event) {
		...
		if(event.getPlayer().hasPermission("shortify.shorten")) {
			...
			try {
				if(plugin.getConfig().getString("mode", "replace").equals("replace")) {
					event.setMessage(new ChatArguments(shortenAll(msg, Integer.valueOf(minlength), getShortener(plugin.getConfig()))));
				}
				else if(plugin.getConfig().getString("mode", "replace").equals("classic")) {
					Engine.STANDARD_BROADCAST_CHANNEL.broadcastToReceivers(new ChatArguments(classicUrlShorten(msg, Integer.valueOf(minlength), getShortener(plugin.getConfig()))));
				}
